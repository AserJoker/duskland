...
void text::set_text(const std::wstring &text) {
  ...
  attr.size.height = 1;
  for (auto &c : text) {
    attr.size.width += util::wcwidth(c);
  }
  attr.size.width = util::wcswidth(text.c_str(), text.length());
  request_update();
}
...
void text::on_update() {
  ...
  if (_auto_wrap && get_parent() &&
      get_parent()->get_attribute().yoverflow == style::FIXED) {
    auto &attr = get_attribute();
    attr.size.height = 1;
    for (auto &c : text) {
      attr.size.width += util::wcwidth(c);
